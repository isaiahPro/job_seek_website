...
const Com_ForgetPassword = () => {
  ...
  const handleSubmit1 = async (e) => {
    ...
    setIsloading(true);
    try {
      const response = await sendCompanyVerification(email);
      if (response) {
        setVerify_page(true);
        setIsloading(false);
      }
  ...
  const handleSubmit2 = async (e) => {
    ...
    setIsloading2(true);
    try {
      const response = await changeCompanyPassword(
        email,
        verificationCode,
        newPassword
      );
    ...
    <div className={"pt-20"}>
      {verify_page ? (
          ...
          >
            <div className={"px-20 flex flex-row justify-center"}>
              ...
              <input
                ...
                className={
                  "input input-sm bg-transparent rounded-md border w-[70%] border-slate-300"
                }
              />
            </div>
            ...
            {errorMessage && (
              <p
                className={
                  "text-error mx-36 mt-7 font-semibold text-xs font-roboto"
                }
              >
                {errorMessage}
              </p>
        ...
        >
          ...
          <button type="submit" className={"mx-auto mt-5 text-center w-full"}>
            {!isloading ? (
              <div
                className={
                  "text-base font-semibold text-center bg-blue-700 w-fit mx-auto p-2 text-white rounded-md hover:opacity-85 font-ubuntu "
          ...
          {errorMessage && (
            <p
              className={
                "text-error mt-7 font-semibold text-xs pl-5  font-roboto"
              }
            >
              {errorMessage}
            </p>
...
export default Com_ForgetPassword;
